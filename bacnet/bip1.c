...
//	BIP_send_conn = uip_udp_new(&addr, HTONS(UDP_BACNET_LPORT));
//	if(BIP_send_conn != NULL) { 
//		uip_udp_bind(BIP_send_conn, HTONS(UDP_BACNET_LPORT));
//			
//  }
...
//	uip_udp_new(&addr, HTONS(response_scan_port));
} 
...
//	u8  n = 0;
	...
	{  // auto send
		...
		{		
			if(Send_mstp_Flag)
			{
				uip_send(mstp_bac_buf.buf,mstp_bac_buf.len);	
				Send_mstp_Flag = 0;
			}
 ...
 //   (void) npdu_data;
	...
	if(protocal == BAC_IP)
	{
		...
		if(Send_bip_Flag)
		{
			...
			uip_ipaddr(addr, 255,255,255,255);	
			bip_send_conn = uip_udp_new(&addr, HTONS(UDP_BACNET_LPORT)); // des port
			if(bip_send_conn != NULL) 
			{ 
				uip_udp_bind(bip_send_conn, HTONS(UDP_BIP_SEND_LPORT));  // src port					
			}
			
			memcpy(bip_bac_buf.buf,&mtu, mtu_len);
