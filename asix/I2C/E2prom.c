...
*/
//const U8_T CPU_Config_CM5[40] = 
//{
//	0x21,0xfc,0x00,0x00,0x30,0x00,0x01,0x00,
...
void E2prom_Initial(void)
{
	...
	ADDROFDEV = 0x50;
	delay_count = 1500;
	memset(&E2_Data,0,sizeof(I2C_BUF));
	switch (sysclk)
	{
		case SYS_CLK_100M :
			Sysclk = 100000000;
...
{
	if(I2C_RdmRead(ADDROFDEV, addr, &E2_Data, 1, I2C_STOP_COND) == TRUE)
	{
		*value = E2_Data.I2cData[0];
		return 1;
	}
	else 
	{
...
//	*value = temp + temp1 * 256;
	if(I2C_RdmRead(ADDROFDEV, addr, &E2_Data, 2, I2C_STOP_COND) == TRUE)
	{
		*value = E2_Data.I2cData[0];			   
		return 1;
	}
...
U8_T E2prom_Write_Byte(U8_T addr,U8_T dat)
{
	...
	if(I2C_ByteWrite(ADDROFDEV, addr, dat, I2C_STOP_COND) == TRUE)
	{
...
{	
	...
	if(I2C_ByteWrite(ADDROFDEV, addr, temp, I2C_STOP_COND) == TRUE)
	{
	for(i = 0; i < delay_count; i++) 			_nop_ ();
	temp = dat >> 8;
	}
	...
	if(I2C_ByteWrite(ADDROFDEV, addr + 1, temp, I2C_STOP_COND) == TRUE)	
	{
	for(i = 0; i < delay_count; i++) 			_nop_ ();
	return 1;
	}
...
//	{
//		#if MINI
//		I2C_ByteWrite(ADDROFDEV, loop, CPU_Config_MINI[loop], I2C_STOP_COND);
//		#else
//		I2C_ByteWrite(ADDROFDEV, loop, CPU_Config_CM5[loop], I2C_STOP_COND);
//
//		#endif
//	}
//}
