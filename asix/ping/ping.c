...
{
	tsPING_LINK pEntity;
	tsPING_CONFIG_SET *pCfig = (tsPING_CONFIG_SET *)pEntity.Buf;
...
void PING_Send(tsPING_CONFIG_SET *pEntity)
{
	U16_T i, j;
	tsIP_HEADER far *pIP;
	tsICMP_HEADER far *pICMP;	
  Test[24]++;
	/* append ICMP header */
	
	pICMP = (tsICMP_HEADER far*)(TCPIP_GetXmitBuffer() + ETH_HEADER_LEN + 20);
	...
	/* append IP header */
	pIP = (tsIP_HEADER far*)(TCPIP_GetXmitBuffer() + ETH_HEADER_LEN);
