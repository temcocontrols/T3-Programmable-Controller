...
//							if(Modbus.mini_type == MINI_TINY) 
					...
					{								
						...
						//Test[25 + i] = AO_feedback[i];
								//						13;  // 13 is  0.04v 的内阻;
						if(AO_feedback[i] < 450)
							AO_feedback[i] += 1000 / AO_feedback[i];
						else
							AO_feedback[i] -= AO_feedback[i] / 50;						
						//Test[35 + i] = AO_feedback[i];
//						AO_feedback[i] = (U32_T)AO_feedback[i] * 100 / 94 + 13;  // 13 is  0.04v 的内阻
//						
					}					
				}
			}
