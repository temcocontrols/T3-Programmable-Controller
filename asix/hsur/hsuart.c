...
U16_T far uart2_timeout = UART2_TIMEOUT;
...
extern volatile U32_T data xTickCount;
		...
		for (i=0 ; i<hsurRxTrigLvl ; i++)
		{
			...
			{	
				...
				if(Modbus.com_config[2] == BACNET_SLAVE || Modbus.com_config[2] == BACNET_MASTER)
					FIFO_Put(&Receive_Buffer0, hsurRxBuffer[hsurRxCount - 1]);
...
void HSUR_Setup(U16_T divisor, U8_T lCtrl, U8_T intEnb, U8_T fCtrl, U8_T mCtrl)
{
	...
	hsurErrTimeStop = 0;
	hsurErrBlocking = 0;
	hsurBaudRateDivisor = 0;
	hsurLineStatusValue = 0;
	hsurModemStatusValue = 0;	
	hsurLineControlValue = 0;
	hsurModemControlValue = 0;
	hsurFifoControlValue = 0;
	hsurAppFlowOn = 0;
