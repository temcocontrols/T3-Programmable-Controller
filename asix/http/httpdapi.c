...
/*
 * ----------------------------------------------------------------------------
 * Function Name: U8_T HTTPAPI_Type2String(U8_T type, U8_T* pBuf)
 * Purpose: Transfer a string to ulong number.
 * Params:
 * Returns:
 * Note:
 * ----------------------------------------------------------------------------
 */
/*  fun for string device name */
U8_T HTTPAPI_Type2String(U8_T type, U8_T* pBuf)
{
	char * bb = "Big MiniPanel";
	char * lb = "Small MiniPanel";	
	
	if(type == 0x01)
	{
			strcpy(pBuf,bb);
			return (U8_T)strlen(bb);
	}else
	{
			strcpy(pBuf,lb);
			return (U8_T)strlen(lb);				
	}	

} /* End of HTTPAPI_Type2String() */


/*
 * ----------------------------------------------------------------------------
 * Function Name: U8_T HTTPAPI_MAC2String(U8_T* mac, U8_T* pBuf)
 * Purpose: Transfer a string to ulong number.
 * Params:
 * Returns:
 * Note:
...
U8_T	HTTPAPI_MAC2String(U8_T* mac, U8_T* pBuf)
{
		U8_T	i = 0;
		U8_T  temp1;		
		...
		{
			temp1 = mac[i] / 16; 
			pBuf[3*i] = valueTable[temp1];	
			temp1 = mac[i] % 16;
			pBuf[3*i + 1] = valueTable[temp1];		
			pBuf[3*i + 2] = 0x2d;
		}
...
/*
 * ----------------------------------------------------------------------------
 * Function Name: HTTPAPI_ULong2Str
 * Purpose: Transfer a unsigned Long number to decimal string.
 * Params:
 * Returns:
 * Note:
 * ----------------------------------------------------------------------------
 */
U8_T HTTPAPI_ULong2Str(U32_T value, U8_T* pBuf)
{
	U8_T	i, j, k, len = 0;
	U32_T	tmp;
