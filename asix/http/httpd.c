...
extern void uart_send_string(U8_T *p, U16_T length,U8_T port);
		...
		for (x = 0; x < MAX_POST_RECORDS; x++)
		{
			if (POST_Record[x].Occupy == 1)
			{
				...
				if (y == POST_Record[x].NameLen)
				{
					...
					switch (POST_Record[x].PostType)				 //here!
					{
...
//		printd ("Post Table %bx\n\r", HTTP_PostTable[i]);
		...
		//	uart_send_string("TEXT\r\n.",8,0);	
			...
			if (!change)	 // no change jump alway
				continue;
...
//		   uart_send_string(POST_Record[HTTP_PostTable[i]].Name,POST_Record[HTTP_PostTable[i]].NameLen,0);	
//		   uart_send_string(pText->CurrValue,pText->UserLength,0);	
//		   uart_send_string("\r\n.",2,0);
	...
	//		uart_send_string("SELE\r\n.",8,0);
			...
			if (pSelect->UserSet != pSelect->CurrentSet)
			{
				...
				change = 1;
				//uart_send_string(&(pSelect->UserSet),1,0);
			}
