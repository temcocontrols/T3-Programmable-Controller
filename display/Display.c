...
#if (ARM_MINI || ASIX_MINI || ARM_CM5 || ASIX_CM5)
...
//	U8_T temp1, temp2;
	...
	Lcd_Show_String(0, 0, text, NORMAL, 21,0,0);
	// subnet mask address		
...
//	Lcd_Show_String(2, 0, text, NORMAL, 21);
	...
	Lcd_Show_String(1, 0, text, NORMAL, 21,0,0);
	sprintf(text, "DB: %u ON: %u", (uint16)sub_no,(uint16)current_online_ctr);
	Lcd_Show_String(2, 0, text, NORMAL, 21,0,0);
...
void Display_Idle(U8_T index)
{
	...
	if(index == 1)
	{
		...
		Lcd_Show_String(2, 0, text, NORMAL, 21,0,0);
		// tcp port
		sprintf(text, "PORT: %u", (U16_T)Modbus.tcp_port);
		Lcd_Show_String(3, 0, text, NORMAL, 21,0,0);
		// MAC address
		...
		Lcd_Show_String(4, 0, text, NORMAL, 21,0,0);
