...
extern volatile char xTickCount;
	...
	if (hsurFlowCtrl == UR2_FLOW_CTRL_SW)
	{
		...
		if (hsurFlowCtrl == UR2_FLOW_CTRL_SW)
		{
			...
			while (hsurTxTransmit == 0)
			{
				...
				{
					printf("Can not transmit in UR2_FLOW_CTRL_SW because of no UR2_THRI_INTR\n\r");
				}
			}
			...
			while (hsurTxState == 1)
			{
				...
				if (((tIdleStop - tIdleStart) / SWTIMER_COUNT_SECOND) >= 3)
				{
					printf("Do not receive any flow control character in UR2_FLOW_CTRL_SW\n\r");
				}
			}
	...
	if (hsurFlowCtrl == UR2_FLOW_CTRL_HW)
	{
		...
		while (1)
		{
			...
			else
			{
				...
				{
					printf("Do not match any flow control character in UR2_FLOW_CTRL_HW\n\r");
				}
			}
		}
	}
		...
		if (hsurFlowCtrlXoff == TRUE)
		{
			...
			while (1)
			{
				...
				else
				{
					...
					if (((tIdleStop - tIdleStart) / SWTIMER_COUNT_SECOND) >= 3)
					{
						printf("Do not match any flow control character in UR2_FLOW_CTRL_X\n\r");
					}
				}
			}
		}
	...
	else if (hsurFlowCtrl == UR2_FLOW_CTRL_SW)
	{
		...
		while (1)
		{
			...
			else
			{
				...
				if (((tIdleStop - tIdleStart) / SWTIMER_COUNT_SECOND) >= 3)
				{
					printf("Do not match any flow control character in UR2_FLOW_CTRL_SW\n\r");
				}
			}
		}
	}
...
void HSUR_ErrorRecovery(void)
{
	if (hsurErrBlocking)
	{
		...
		{
			printf("clear hsurErrBlocking = 0\n\r");
			EA = 0;
