...
extern U8_T SD_SendCmd(U8_T cmd, U8_T *param, U8_T resptype, U8_T *resp);   // 一个SPI命令   a SPI command
extern void SD_PackParam(U8_T *parameter, U32_T value);						// 封装参数	  	 pack the parameter
extern U8_T SD_BlockCommand(U8_T cmd, U8_T resptype, U32_T parameter);		// 块命令 		 block command 

	
extern U8_T SD_ResetSD(void);												// 复位SD卡			reset SD Card
//extern U8_T SD_CheckSDVer(void); 											// 检测SD卡版本		check the SD version

extern U8_T SD_ReadCSD(U8_T csdlen, U8_T *recbuf);							// 读CSD 			read CSD register
extern U8_T SD_ReadCID(U8_T cidlen, U8_T *recbuf);							// 读CID			read CID register
extern U8_T SD_StopTransmission(void);										// 停止传输			stop transmission

extern U8_T SD_ReadCard_Status(U8_T *buffer);								// 读Card Status	read Card Status register
extern U8_T SD_SetBlockLen(U32_T length);									// 设置块长度		set block length

extern U8_T SD_ReadSingleBlock(U32_T blockaddr);							// 读单块			read a single block
extern U8_T SD_ReadMultipleBlock(U32_T blockaddr);							// 读多块			read multiple block

extern U8_T SD_WriteSingleBlock(U32_T blockaddr);							// 写单块			write a single block
extern U8_T SD_WriteMultipleBlock(U32_T blockaddr);							// 写多块 			write multiple block


extern U8_T SD_ProgramCSD(U8_T len, U8_T *buff);						// 写CSD寄存器  	write CSD register 

extern U8_T SD_EraseStartBlock(U32_T startblock);						// 擦块起始地址		erase start address
extern U8_T SD_EraseEndBlock(U32_T endblock);							// 擦块终止地址		erase end address
extern U8_T SD_EraseSelectedBlock(void);								// 擦除所选的块		erase selected address


extern U8_T SD_ReadOCR(U8_T *recbuf);					// 读OCR			read OCR register
extern U8_T SD_EnableCRC(U8_T bEnable);								// 使能CRC校验		enable CRC


// 特殊应用命令
extern U8_T SD_ReadSD_Status(U8_T sdslen, U8_T *recbuf);				// 读SD_Status		read SD_Status 
extern U8_T SD_GetNumWRBlcoks(U32_T *blocknum);						// 得到正确写入块数 get block numbers that writed well
extern U8_T SD_ReadSCR(U8_T scrlen, U8_T *recbuf);					// 读SCR 			read SCR register


// 数据流函数
extern U8_T SD_ReadRegister(U32_T len, U8_T *recbuf);						// 读寄存器  read register 
extern U8_T SD_ReadBlockData(U32_T len, U8_T *recbuf);						// 读块数据	 read block data 
extern U8_T SD_WriteBlockData(U8_T bmulti, U32_T len, U8_T *sendbuf);		// 写块数据	 write block data

// 其它函数
extern void SD_StopMultiToken(void);									// 停止多块写令牌	 stop token when write multiple block
extern U8_T SD_WaitBusy(U8_T waittype);								// 忙				 busy
extern void SD_SPIDelay(U8_T value);									// 发生SPI时钟		 send SPI clock
